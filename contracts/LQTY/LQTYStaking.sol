...
contract LQTYStaking is ILQTYStaking, Ownable, CheckContract, BaseMath {
    ...
    // --- Data ---
    string constant public NAME = "CALStaking";
    ...
    function _sendETHGainToUser(uint ETHGain) internal {
        ...
        require(success, "CALStaking: Failed to send accumulated BTCGain");
    }

    // --- 'require' functions ---

    function _requireCallerIsTroveManager() internal view {
        require(msg.sender == troveManagerAddress, "CALStaking: caller is not TroveM");
    }

    function _requireCallerIsBorrowerOperations() internal view {
        require(msg.sender == borrowerOperationsAddress, "CALStaking: caller is not BorrowerOps");
    }

     function _requireCallerIsActivePool() internal view {
        require(msg.sender == activePoolAddress, "CALStaking: caller is not ActivePool");
    }

    function _requireUserHasStake(uint currentStake) internal pure {  
        require(currentStake > 0, 'CALStaking: User must have a non-zero stake');  
    }

    function _requireNonZeroAmount(uint _amount) internal pure {
        require(_amount > 0, 'CALStaking: Amount must be non-zero');
    }
