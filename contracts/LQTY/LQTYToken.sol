...
contract LQTYToken is CheckContract, ILQTYToken {
    ...
    string constant internal _NAME = "Cataliza";
    string constant internal _SYMBOL = "CAL";
        ...
        public 
    {
        ...
        uint depositorsAndFrontEndsEntitlement = 2079e22; // Allocate 20.79M to the algorithmic issuance schedule
        _mint(_communityIssuanceAddress, depositorsAndFrontEndsEntitlement);

        uint _lpRewardsEntitlement = 21e22;  // Allocate 210k (1%) for LP rewards 
    ...
    function sendToLQTYStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsLQTYStaking();  // Prevent the multisig from staking LQTY
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'CAL: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'CAL: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "CAL: Cannot transfer tokens directly to the CAL token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != lqtyStakingAddress,
            "CAL: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }

    function _requireCallerIsLQTYStaking() internal view {
         require(msg.sender == lqtyStakingAddress, "CALToken: caller must be the CALStaking contract");
    }
