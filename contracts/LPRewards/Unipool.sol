...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
    ...
    function rewardPerToken() public view override returns (uint256) {
        ...
        return
        // (( 1650454400 - 1640454400 ) * 0.006658211794546607482562 ) * 1e18 / 1000
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == lqtyToken.balanceOf(address(this))); // 210k
        assert(periodFinish == 0);

        _updateReward();

        // Say, Start:1630454400 , End: 1661990400, 
        // 31540000 ( 1 year )
        rewardRate = _reward.div(_duration); // 210k / 31540000 = 0.006658211794546607482562
